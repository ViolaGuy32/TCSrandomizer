...
//    wxLogStatus("Generating files. . .");
    ...
    lineDeleter(LEV + "AREAS.TXT", {
				       29,  45,  63,  65,  94,  95,  96,  97,  114, 115,
				       201, 220, 221, 383, 456, 
				       512, 546, 547, 548, 600, 601, 602, 
				       690, 737, 
				   });

    //lineDeleter(out + Negotiations->path + "NEGOTIATIONS_INTRO/EP1_FAILEDNEG_INTRO2.TXT", {1, 2, 3});
    //lineDeleter(out + BHP->path + "PURSUIT_INTRO/PURSUIT_INTRO.TXT", {1});
    ...
    speedUp(2, "COUNTDOOKU_INTRO", {1, 10});
    ...
    speedUp(4, "MOSEISLEY_MIDTRO", {1, 4});
