...
void Randomize(std::mt19937 rando) {
	...
	std::array<std::string, 36> extras = {"supergonk", "poomoney", "walkietalkiedisable",
		"powerbrickdetector", "superslap", "forcezipup", "coinmagnet", "disarmtroopers",
		"characterstuds", "perfectdeflect", "explodingblasterbolts", "forcepull",
		"vehiclesmartbomb", "superastromech", "superjedislam", "superthermaldetonator",
		"deflectbolts", "darkside", "superblasters", "fastforce", "supersabres", "tractorbeam",
		"invincibility", "scorex2", "selfdestruct", "fastbuild", "scorex4", "regenerate", "scorex6",
		"minikitdetector", "superzapper", "rockets", "scorex8", "superewokcatapult", "scorex10",
		"infinitetorpedos"};
	...
	if (collectable) {
		for (int i = 0; i < 36; i++) {
			int q = 0;
			std::array<char, 21> collect = {'m', 'm', 'm', 'm', 'm', 'm', 'm', 'm', 'm', 'm', 'c',
				'c', 'c', 'c', 'c', 'c', 'c', 'c', 'c', 'c', 'r'};
...
negotiations:
	...
	if (logicType == casual) {
		...
		if (!MultiAny({Jump, Flutter, Hovering}, 2)) {
			bool NoJumpPast = 0;
			//moving during the camera pan with a nonjumper
			for (Playable* p : testing) {
				if (p == mousedroid || p == destroyer) NoJumpPast = 1;
			}
			goto negotiations;
		}
...
theed:
	...
	if (logicType == casual) {
		...
		for (Playable* p : testing) {
			...
			if (atrb(LukeAlt) && p == gamorreanguard) overGap.push_back(p);
		}
		std::vector<Playable*> pastDoor;
		if (overGap.size() == 6) pastDoor = overGap;
		if (logicType == superGlitched) {
			if (overGap.size() >= 5) pastDoor = overGap;
			else {
				for (Playable* p : overGap) {
					for (Playable* x : overGap) {
						if (p != x) {
							if (x->jedi && p->pushable) pastDoor.push_back(p);
							if (x->choke && p->chokeable) pastDoor.push_back(p);
							if (x->lightning && p->lightningable) pastDoor.push_back(p);
							if (x->lightning && p->resistZap) pastDoor.push_back(p);
							if (x->zapper && p->zappable) pastDoor.push_back(p);
							if (x->jedi && p->trickable) pastDoor.push_back(p);
							if (x->astrozapper && p->storm) pastDoor.push_back(p);
							if (x->landoAlt && p->leiaAlt) pastDoor.push_back(p);
							if (x->lukeAlt && p == gamorreanguard) pastDoor.push_back(p);
						}
					}
				}
			}
		...
		if (SuperJump({}, pastDoor)) goto theed2;
		if (panel(1, 0, pastDoor)) goto theed2;
	...
	theed2:
		...
		if (!atrb(HighJump) && !atrb(Jedi) && !SuperJump({Jump, Flutter}) &&
			!All({Shoot, DoubleJump}))
			goto theed; //Theed 1. Shoot, DoubleJump is for Ackbar
		goto maul;
	}
	goto theed;
...
chancellor:
	...
	if (!panel(0, 3) && logicType != superGlitched) goto chancellor; //top of room
	if (!panel(0, 3) && !LivingJedi()) goto chancellor;       //top of room
	...
	if (logicType != casual && panelOr(3, 0, {DoubleJump, Dive, Flop, SlightlyBetterJump}))
		gasOff = true;
...
kashyyyk:
	...
	if (logicType == casual) {
		...
		if (!MultiAny({Jump, Flutter}, 2))
			goto kashyyyk; //to get onto cliff thing for buttons, launch is TAS-only
		if (logicType != superGlitched) {
...
{
	bool plansthinghallway = false;
	mix(SecretPlans);
	auto plansthing = [&plansthinghallway](
		...
		for (Playable* x : current) {
			for (Playable* y : current) {
				...
				if (x != y) {
					...
					else if (x->lightningable && y->resistZap) OOB = true;
					...
					if (OOB) {
						for (Playable* x2 : current) {
							for (Playable* y2 : current) {
								if (x2 != y2 && x2 != x) {
									...
									else if (x2->lightningable && y2->resistZap) OOB2 = true;
						...
						if (panel(2, 2, {x, shield})) {
							plansthinghallway = true;
							return true; //end of hallway
						}
						if (panel(2, 1, {x, otherX}) && Any({Box, Bounty}, {x, otherX}))
							return true; //into last scene
						if (panel(2, 1, {redGuy}) &&
	...
	if (logicType == casual) {
		if (!All({Grapple, Build}) && !All({YodaJump, Build}) && !All({ExtraHighJump, Build}))
			goto secretplans;
		...
		if (!atrb(Lever)) goto secretplans;
		if (((All({Grapple, Build}) || atrb(Jedi) || //All Jedi can build.
				 All({ExtraHighJump, Build})) &&
				(atrb(Shoot) ||
					atrb(FakeShoot) /*|| atrb(Block)*/)) || //You can't actually deflect into the
															//bombs without EBB.
			SuperJump({Jump, Flutter}) ||
			DoubleTransitionSkip({Jump, Flutter}))
			goto secretplans2;
		goto secretplans;
	}
secretplans2:
	...
	if (logicType == casual) {
		...
		//panels for car not included because of the transition skip
		if (Any({Jedi, Bounty}) && panel(2, 2, {SecretPlans->party[4]})) goto secretplans3;
		if ((atrb(Jedi)  || atrb(Bounty)) && panelAnd(2, 2, {Jump, Flutter})) goto secretplans3;

		if (atrb(Hat) && availableHats[0] == BountyHat && panel(2, 2, {SecretPlans->party[4]}))
			goto secretplans3;

		if (panel(2, 0) &&
			panel(2, 2, {SecretPlans->party[1], SecretPlans->party[3], SecretPlans->party[4]}))
			goto secretplans3;

		//droids and rebel friend area with super jumps
		if (plansthing({SecretPlans->party[0], SecretPlans->party[1], SecretPlans->party[3]},
				SecretPlans->party[4], SecretPlans->party[2]))
			goto secretplans3;

		if (panel(2, 0) &&
			plansthing({SecretPlans->party[1], SecretPlans->party[3], SecretPlans->party[4]},
...
jundland4:
	...
	if (logicType == casual) {
		...
		if (InstantSuperJump()) //cannot go directly into elevator
			goto jundland2;
		goto jundland;
	}
jundland2:
...
dse:
	...
	if (!Multi(Lever, 2)) goto dse; //maybe not in glitched?

	//first area
	if (SuperJump({Jump})) goto dse3;
	if (logicType == superGlitched) { //Box Clip with the Service Car.
		if (All({Grapple, Lever}) || All({DoubleJump, Lever})){
			if (atrb(Box)) goto dse3;
		}
	}
...
echobase2:
	...
	if (logicType == superGlitched) goto echobase3; //BOOM Skip
...
dagobah:
	...
	if (logicType == casual) {
		...
		if (MultiAny({Jump, Fly, Flutter, Tall, Hovering}, 2) && atrb(AstroZapper))
			goto dagobah2; //test
...
cct:
	...
	cctGlitch:
		...
		if (!panel(0, 4) || !logicType == superGlitched) goto cctGlitch;
		...
		if (panel(0, 5)) {
			...
			if (panel(0, 6, {darthvader})) { //need Vader to open the panel
				for (Playable* p : testing) {
					for (Playable* q : testing) {
						if (p != q) {
							if (p->fly || p->realDoubleJump) {
								if (q->attack) goto cct2;
							}
						}
					}
				}
		...
		if (panel(0, 6, dv1) || SuperJump({}, dv1)) goto cct2; //can super jump to skip panel
		//if (panel(0, 7, dv1)) {
		//raise platform
	...
	cct2:
		...
		if (CCT->panels[2].panels[1].type == AstroPanel && panel(2, 1) &&
			...
			goto cct3;
		if (SuperJump({Jump, Fly})) goto cct3; //DVT
...
bespin:
	...
	if (!(atrb(Lever) && Any({Attack, FakeShoot})) && !atrb(DoubleJump) && !atrb(Flutter) &&
		!atrb(Hovering))
		goto bespin;

	{
		...
		while (!room2) {
			...
			if (!gotr2 && pastFight.size() != 0) {
				...
				if (panel(0, 4, temp)) {
					...
					//superJump into BOOB room
					if (logicType == superGlitched && Multi(Gas, 2)) {
						add(3);
						pastFight.push_back(Bespin->party[3]);
						gotr2 = true;
						goto bespin3po;
		...
		bespin3po:
			...
			if (panel(0, 6, pastDoor, tempHats) && Multi(Jump, 2) && logicType != casual)
				room2 = true; //using the other elevator to clip
...
jabbas:
	...
	if (SuperJump({Jump, Flutter})) goto jabbas2; //hit transition from OOB
				...
				if (logicType == superGlitched) {
					for (Playable* x : testing) {
						if (p != x) {
							...
							if (x->lightning && p->resistZap) pastGate.push_back(p);
...
jabbas3: //long room
	...
	if (logicType == casual) {
		...
		if (SuperJump({Jump, Flutter})) goto jabbas5; //rancor warp
		goto jabbas;
	}

jabbas4:
	//panel or rancor warp backup
	if (atrb(DoubleJump) || panel(2, 3)) goto jabbas5;
	goto jabbas;
...
jabbas5: //rancor
	...
	add(3);
	add(4);
	if (!panel(3, 0) && !panel(3, 1) && !panel(3, 0, {Jabbas->party[5]}, {}) &&
		!panel(3, 1, {Jabbas->party[5]}, {})) {
...
carkoon2: //second skiff
	...
	if (logicType == casual) {
		...
		carkoon4:
			...
			if (Multi(Lever, 2)) goto carkoon5;
			...
			goto carkoon;
		carkoon5:
			if (Any({DoubleJump, Fly, Dive})) goto carkoon3;

			//need good enough run_speed to do the jump from the rope
			for (Playable* p : testing) {
				if (p->jump && p->speed >= 1.199) goto carkoon3;
			}
...
bhm:
		...
		for (Level* lev : allLevels) {
			for (EnemySet& enSet : lev->enemies) {
				for (Enemy& en : enSet.enemy) {
					if (std::find(lev->enemyTypes.begin(), lev->enemyTypes.end(), en.newEn) ==
						lev->enemyTypes.end()) {
						lev->enemyTypes.push_back(en.newEn);
					}
				}
			}
			for (SpecialScp& sp : lev->specialscp) {
				for (Enemy& spEn : sp.specialEnemies) {
					if (std::find(lev->enemyTypes.begin(), lev->enemyTypes.end(), spEn.newEn) ==
						lev->enemyTypes.end()) {
						lev->enemyTypes.push_back(spEn.newEn);
					}
...

	{
		...
		int addresses[] = {0x4f13, 0x4f33, 0x4fc6, 0x1a7af, 0x29322, 0x895e9, 0xca382, 0x1de5f,
			0x31774, 0x8727a, 0x317a2, 0x317b2, 0x1f29f, 0x2d314, 0x400a6, 0x400c2, 0x400c9,
			0x400e5, 0x4005c, 0x40078, 0x400ec, 0x40108, 0x4007f, 0x317c2, 0xb57e5, 0x87288,
			0xa91ca, 0x1e3e2, 0x1e3eb, 0xa91d7, 0x40039, 0x40055};
	...
	/*if (enemyOp) {
		...
		for (Level* lev : allLevels) {
			...
			lineDeleter(tf, lev->enemyRes);

			for (Playable* p : lev->enemyTypes) {
	...
	*/
		...
		for (Level* lev : allLevels) {
			for (PanelSet& panSet : lev->panels) {
				for (Panel& pan : panSet.panels) {
					...
					if (pan.type == AstroPanel || pan.type == ProtoPanel) {
						binaryWrite(
							getBasePath(lev, panSet.scene, "GIZ"), pan.altColor, pan.address + 19);
						binaryWrite(
							getBasePath(lev, panSet.scene, "GIZ"), pan.altBody, pan.address + 20);
					}
				}
			}
		}
	...
	if (collectable) {
			...
			for (SpecialCollectable& collect : allLevels[i]->specialCollectables) {
				...
				for (std::pair<char, int> special : collect.sceneAddress) {
					...
					if (type == 'c') {
						...
						//breaks when changed to minikits
						hexWrite(
							file, "c_pup" + std::to_string(challengeNum), special.second - 0x14);
						++challengeNum;
					}
		...
		lineDeleter(
			getBasePath(Coruscant, 'A', "GIT"), {274, 302, 330, 358, 386, 414, 442, 470, 498, 526});
...
#endif
		unsigned int lines[] = {39, 57, 75, 88, 128, 144, 195, 214, 233, 246, 259, 286, 332, 351,
			363, 377, 392, 406, 468, 485, 505, 523, 540, 562, 615, 632, 652, 669, 684, 700, 749,
			764, 777, 793, 806, 829};
...
#define bonusCharacter &Level::bonusCharacters
		...
		playerInit({{0, 1}, {1, 2}, {2, 3}, {3, 4}, {4, 5}, {2, 8}, {3, 9},
			{4, 10}}); //so they still spawn in FP

		scpMany('B', "PARTY",
			{{2, {{15, 11}, {31, 11}}}, {3, {{11, 11}, {26, 11}}}, {4, {{7, 11}, {21, 11}}}});
		...
		playerInit({{0, 1}, {1, 2}, {2, 3}});
		scpMany(
			'C', "DOOKU", {{0, {{209, 45}}}, {1, {{207, 26}, {214, 29}}}, {2, {{208, 43}}}}); //Yoda
		...
		scpMany('A', "PARTY",
			{{0, {{23, 11}}}, {1, {{26, 11}}}, {2, {{29, 11}, {14, 11}}}, {3, {{17, 11}}},
				{4, {{20, 11}}}});
		scpMany('B', "PARTY",
			{{0, {{23, 11}}}, {1, {{26, 11}}}, {2, {{29, 11}, {14, 11}}}, {3, {{17, 11}}},
				{4, {{20, 11}}}});
		scpMany('C', "PARTY",
			{{0, {{7, 11}, {10, 11}}}, {2, {{34, 11}}}, {3, {{28, 11}, {53, 11}}},
				{4, {{31, 11}}}});
		scpMany('D', "PARTY",
			{{0, {{38, 26}}}, {1, {{8, 11}}}, {2, {{9, 11}}}, {3, {{6, 11}}}, {4, {{7, 11}}}});
		...
		scpMany(
			'D', "PARTY", {{2, {{6, 11}, {9, 11}, {32, 11}}}, {3, {{7, 11}, {12, 11}, {35, 11}}}});
		...
		scpMany('A', "PARTY",
			{{0, {{8, 11}}}, {1, {{11, 11}}}, {2, {{14, 11}}}, {3, {{17, 11}}}, {4, {{24, 11}}},
				{5, {{21, 11}}}});
		scpMany('B', "PARTY",
			{{0, {{8, 11}}}, {1, {{11, 11}}}, {2, {{14, 11}}}, {3, {{17, 11}}}, {4, {{24, 11}}},
				{5, {{21, 11}}}});
		...
		scpMany('D', "LEVEL",
			{{0, {{26, 26}, {27, 27}, {28, 27}, {29, 28}, {30, 29}, {108, 26}, {109, 27}, {110, 27},
					 {111, 29}}},
				{1, {{32, 26}, {33, 27}, {34, 27}, {35, 28}, {36, 29}, {113, 26}, {114, 27},
						{115, 27}, {116, 29}}},
				{2, {{38, 26}, {39, 27}, {40, 27}, {41, 28}, {42, 29}, {118, 26}, {119, 27},
						{120, 27}, {121, 29}}},
				{3, {{44, 26}, {45, 27}, {46, 27}, {47, 28}, {48, 29}, {123, 26}, {124, 27},
						{125, 27}, {126, 29}}}});
		...
		scpMany('A', "LEVEL",
			{{3, {{35, 26}, {36, 25}}}, {4, {{33, 26}, {34, 25}}}, {5, {{37, 26}, {38, 25}}}});
		...
		scpMany('B', "LEVEL",
			{{0, {{41, 26}, {42, 27}, {43, 27}, {44, 25}}}, {1, {{59, 28}, {65, 29}, {84, 29}}},
				{2, {{57, 28}, {61, 59}, {63, 29}, {82, 29}, {96, 26}, {97, 27}, {98, 27},
						{99, 25}}},
		...
		scpMany('A', "PARTY",
			{{2, {{28, 11}}}, {3, {{7, 11}}}, {4, {{8, 11}, {10, 11}}}, {5, {{6, 11}}}});
		...
		scpMany('A', "LEVEL",
			{{2, {{132, 27}, {133, 27}, {134, 29}}}, {3, {{130, 59}, {136, 27}, {137, 27}}}});
		scpMany('B', "LEVEL",
		...
		//bounty hunter missions
		{
			...
			batchAnywhere(LEV + "MISSIONS.TXT",
				{{"party \"" + BHM->party[0]->name + "\" \"" + BHM->party[1]->name + "\" \"" +
						 BHM->party[2]->name + "\" \"" + BHM->party[3]->name + "\" \"" +
						 BHM->party[4]->name + "\" \"" + BHM->party[5]->name + "\" ",
					 56, {{1, 1}}},

					{0, {{5, 15}}, bonusCharacter}, {1, {{6, 15}}, bonusCharacter},
					{2, {{7, 15}}, bonusCharacter}, {3, {{8, 15}}, bonusCharacter},
					{4, {{9, 15}}, bonusCharacter}, {5, {{10, 15}}, bonusCharacter},
					{6, {{11, 15}}, bonusCharacter}, {7, {{12, 15}}, bonusCharacter},
					{8, {{13, 15}}, bonusCharacter}, {9, {{14, 15}}, bonusCharacter},
			...
			auto missionReplace2 = [](int c, unsigned int line, Level* lev, char scene,
									   std::string script) {
				writer(oneWrite, getSCP(lev, scene, script),
					{BHM->bonusCharacters[c]->name, BHM->vanillaBonusCharacters[c]->name.length(),
						{line, 49}});
		...
		multiPointer(
			indy, {0x2e585, 0xc1053, 0x83b26, 0xc5098, 0x86969, 0x86982, 0xc50d0, 0xc65b1});
...
//Character unlocks
	...
	std::string minis[] = {"mini_republic_cruiser", "mini_gungan_bongo", "mini_royal_starship",
		"mini_sebulba_pod", "mini_naboo_starfighter", "mini_sith_infiltrator",

		"mini_zam_speeder", "mini_jedi_starfighter", "mini_droideka", "mini_gunship", "mini_atte",
		"mini_solar_sailor",

		"mini_dropship", "mini_emergency_ship", "mini_e3_jedi_starfighter", "mini_wookie_cat",
		"mini_e3_x_wing", "mini_v_wing",

		"mini_star_destroyer", "mini_sand_crawler", "mini_land_speeder", "mini_millennium_falcon",
		"mini_y_wing", "mini_tie_advanced",

		"mini_atat", "mini_snow_speeder", "mini_tie_fighter", "mini_x_wing", "mini_slave1",
		"mini_cloud_car",

		"mini_desert_skiff", "mini_sail_barge", "mini_tie_bomber", "mini_atst",
		"mini_tie_interceptor", "mini_imperial_shuttle"};
	...
	*/
	...
	if (colorOp) {
		...
		rgbBatch(PTL, blue,
			{0x4A0, 0x4A8, 0x591A, 0x5922, 0x592A, 0x77D2, 0xF5FD, 0xF689, 0xF691, 0x8357});
		rgbBatch(PTL, green, {0x877, 0x87F, 0x5543, 0x554B, 0x5553, 0xAD94});
		rgbBatch(
			PTL, red, {0x60C8, 0x60D0, 0x60D8, 0xB542, 0xC0C7, 0xC0CF, 0xFDAB, 0xFE37, 0xFE3F});
